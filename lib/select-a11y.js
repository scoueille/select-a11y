        ...
        function transform($selectToTransform) {
            ...
            function insertScreenReaderDescription() {
                var $screenReaderDescription = $('' +
                    '<p class="sr-only">Utilisez la tabulation (ou la touche flèche du bas) pour naviguer dans la liste des suggestions. Confirmez votre choix avec la touche Entrée ou utilisez la touche Esc pour fermer la liste de suggestions.</p>' +
            ...
            function handleEvents() {
                ...
                $('body').on('click', function (event) {
                    ...
                    if (!$target.is($input) && !$target.is($suggestions.parent()) && !$target.is($suggestions) && !$target.is($revealButton)) {
                        $suggestions.empty();
                        $a11ySelectContainer.addClass('tag-hidden');
